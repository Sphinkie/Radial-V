...
void Plexiglass::setTuning(int tuning)
{
  ...
  Genre = this->getGenreLabel(tuning);   // La valeur textuelle du genre demandé
  Year  = this->getYearValue(tuning);    // La valeur textuelle de l'année demandée
  Rating= this->getStarsValue(tuning);   // La valeur textuelle du rating demandé
}
