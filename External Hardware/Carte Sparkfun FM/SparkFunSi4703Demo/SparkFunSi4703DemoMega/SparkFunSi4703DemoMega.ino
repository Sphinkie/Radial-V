/* *******************************************************************************
 Spark Fun Electronics 2011 - Demo for ARDUINO MEGA
 Nathan Seidle
 This code is public domain but you buy me a beer if you use this and we meet someday (Beerware license).


 To use this code on ARDUINO MEGA, connect the following 5 wires:
 Arduino : Si470x board
    3.3V : VCC
     ...
     D20 : SDIO
     D17 : RST
      A0 : Trimpot tuning (optional)
 
 Look for serial output at 115200 bps.
 ...
 If anyone manages to get this datasheet downloaded http://wenku.baidu.com/view/d6f0e6ee5ef7ba0d4a733b61.html
 ...
 Functionnalities:
   - Display current status (from 0x0A)
   - Add RDS decoding - works, sort of
   - Volume Up/Down
   - Mute toggle
   - Tune Up/Down
   - Read current channel (0xB0)
   - Setup for Europe
   - Seek up/down
...
{                
  ...
  Serial.println("Starting si4703_init");
...
void loop() 
{
  char option;
  char vol = 15;
  int currentChannel = 935; // Default the unit to a known good local radio station (93.5MHz)
  ...
  while(1) 
  {
    ...
    else if(option == '6') 
    {
      ...
      while(1) 
      {
        if(Serial.available()>0) if(Serial.read() == 'x') break;
        si4703_readRegisters();
        if(si4703_registers[STATUSRSSI] & (1<<RDSR))
        {
      ...
      while(1)
      {
        if(Serial.available())
        {
          ...
          if(option == '+') 
          {
            ...
            if(current_vol < 15) current_vol++;          // Limit max volume to 0x000F
...
void gotoChannel(int newChannel)
{
  ...
  Serial.println("Read Si403 regsiters");
  //These steps come from AN230 page 20 rev 0.5
  si4703_readRegisters();
  
  Serial.println("Update Si403 regsiters");
  ...
  si4703_readRegisters();
  si4703_registers[CHANNEL] &= ~(1<<TUNE); // Clear the Tune bit after a tuning has completed
  si4703_updateRegisters();
